...
```
> ros2 launch basic_say basic_say.launch.py
```
...
```
ros2 action send_goal /skill_test/control sample_skill_msgs/action/SkillControl "skill_data: 'test data'" 
```
...
To implement your skill logic, have a look at `basic_say/skill_impl.py`: this is
...
- run `ros2 run pal_deploy deploy --package basic_say generic-pal-XXc` (replace `generic-pal-XXc` by your actual robot)

You can now `ssh` onto the robot (`ssh pal@generic-pal-XXc`, password
`pal`), and go to your project: `cd ~/deployed_ws/share/basic_say`
...
`ros2 run basic_say start_skill`
...
``ros2 lifecycle set /app_basic_say configure``
``ros2 lifecycle set /app_basic_say activate``

To stop it (but not reseting it's internal variables):
``ros2 lifecycle set /app_basic_say activate``

To stop it and clean it:
``ros2 lifecycle set /app_basic_say deactivate``
``ros2 lifecycle set /app_basic_say cleanup``
